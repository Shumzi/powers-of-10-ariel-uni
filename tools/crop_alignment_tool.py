...
class CropAlignmentTool:
    ...
    def save_crop(self):
        ...
        # Save to JSON
        output_file = "crop_alignment_output.json"
        ...
        print(f"Crop region: ({output['crop_region']['x']}, {output['crop_region']['y']}) "
              f"{output['crop_region']['width']}x{output['crop_region']['height']}")
        print("="*60 + "\n")
        
        # Save the cropped image
        cropped_surface = self.base_image.subsurface(self.crop_rect)
        cropped_filename = "cropped_output.png"
        pygame.image.save(cropped_surface, cropped_filename)
        print(f"Cropped image saved: {cropped_filename}\n")
        
        # Close pygame window before running GMIC
        pygame.quit()
        
        # Generate morph sequence using GMIC
        self.generate_morph_sequence(cropped_filename)
    
    def generate_morph_sequence(self, cropped_filename):
        """Generate morph sequence using GMIC"""
        import subprocess
        import shutil
        
        # Create output folder
        output_folder = "morph_sequence"
        os.makedirs(output_folder, exist_ok=True)
        
        # Get the zoomed image path
        # We need to save it if it's not already a file
        zoomed_filename = "zoomed_temp.png"
        ...
        # -x_morph creates a morphing animation between two images
        # Specify number of frames (default is often too low)
        num_frames = 16
        
        # The output needs to come AFTER x_morph processes the images
        gmic_command = [
            gmic_exe,
            cropped_filename,
            zoomed_filename,
