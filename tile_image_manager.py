"""
Tile Image Manager - Drop-in replacement for ImageManager using tile pyramids
Maintains identical API while using pre-computed tiles for fast zoom rendering
"""
from dataclasses import dataclass
import pygame
import os
from image_manager import ImageData, RectData
from tile_pyramid import TilePyramid


class TileImageData(ImageData):
    """Extended ImageData with tile pyramid support"""
    def __init__(self, config, original, surface, scale, bg, max_scale, pyramid=None):
        super().__init__(config, original, surface, scale, bg, max_scale)
        self.pyramid = pyramid  # TilePyramid instance


class TileImageManager:
    """
    Image manager with tile pyramid support for fast zoom rendering.
    
    API-compatible with ImageManager - can be used as drop-in replacement.
    Maintains exact same rect logic and public interface.
    """
    ...
    def __init__(self, config, viewport_dims):
        ...
        self.use_tiles = config.get('use_tile_cache', True)
        
    def load_images(self):
        """Load all images from config and generate tile pyramids"""
        for img_config in self.config['images']:
            ...
            # Generate tile pyramid if enabled
            pyramid = None
            if self.use_tiles:
                pyramid = TilePyramid(original, image_path, self.viewport_dims, scale)
                pyramid.load_or_generate()
                print(f"  Pyramid memory: {pyramid.get_memory_usage() / (1024*1024):.1f} MB")
            
            image_data = TileImageData(img_config, original, scaled, scale, bg, max_scale, pyramid)
